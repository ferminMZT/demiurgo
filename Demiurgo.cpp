...
*/
void funcionEscrituraExterna(ptrIndividuo pInd, ptrConjNodos pConj, fstream &fichero);
...
void main1()
{
 ...
 cout << "Semilla aleatoria empleada:      " << laSemilla << endl;
...
void main2()
{
 ...
 for(n = 0; n < nP; n++)	// Bucle de inicializacion de las poblaciones
 {
  ...
  cout << "Error mejor / Error medio: " << eMI[n] << "/" << eTP[n] << endl;
 ...
 while(nGener < NUM_MAX_GENER)
 {
  ...
  for(n = 0; n < nP; n++)
  {
   ...
   eTP[n] = (*cnjPob)[n]->getErrorMedioPoblacion();
   cout << " #Ind: " << nIG[n] << " Mejor: " << eMI[n] << " Media: " << eTP[n] << endl;
   nIT[n] += nIG[n];
  ...
  for(n = 0; n < nP; n++)	// Recorre todas las poblaciones
   ...
   {				// poblaciones a aquel mas COMPLEMENTARIO, es decir con mayor distancia
    ...
    // && eTP[n] != eTP[nIdoneo])
    {
     cout << "Mejor antes: " << n << endl;
     //(*cnjPob)[n]->getMejorIndividuo()->ver();
     cout << "Elegido otro mejor: " << nIdoneo << endl;
     (*cnjPob)[n]->asignaMejorIndividuo(*((*cnjPob)[nIdoneo]));
     cout << "Mejor despues: " << nIdoneo << endl;
     //(*cnjPob)[n]->getMejorIndividuo()->ver();
    }
    /*
    if(eMI[n] > eMI[nPM])
    ...
    */
    ...
    {
     cout << "Mutacion + Destruccion..." << endl;
 ...
 for(n = 0; n < nP; n++)
 {
  cout << "Poblacion " << n << endl;
  cout << " Mejor: " << *((*cnjPob)[n]->getMejorIndividuo()->getArbol()) << endl;
  cout << " Error mejor: " << (*cnjPob)[n]->getMejorError() << endl;
  (*cnjPob)[n]->actualizaParametros();
  cout << " Error medio: " << (*cnjPob)[n]->getErrorMedioPoblacion() << endl;
  (*cnjPob)[n]->escribeResultados(funcionEscrituraExterna, (int) tiempo.detente());
  cout << " Individuos creados: " << nIT[n] << endl;
  DEBUG("Siguiente");
  nIndTotales += nIT[n];
  delete (*cnjPob)[n];
 }
 cout << "Numero TOTAL de individuos creados: " << nIndTotales << endl;
 DEBUG("Finish!");
 // Destruccion de todo...
 delete cnjPob;
}
...
void main3()
{
 ...
 cout << "Se parte de un error inicial de: " << pueblo.getErrorMedioPoblacion() << endl;
 ...
 while(nGener < NUM_MAX_GENER)
 {
  ...
  cout << "#G" << nGener << " P:" << pueblo.getPosMejor();
  cout << " E:" << errorMejor << "/" << errorMedio << " (" << tpcError << "%)";
  cout << " N:" << pueblo.getMejorIndividuo()->getNumNodos() << "/" << pueblo.getNumMedioNodos();
  cout << " I:" << pueblo.getMejorIndividuo()->getNumIntrones() << "/" << pueblo.getNumMedioIntrones() << endl;
 ...
 tiempo.detente();
 cout << "Tiempo de evolucion: " << tiempo.detente() << " segundos." << endl;
 cout << " Mejor: " << *(pueblo.getMejorIndividuo()->getArbol()) << " : " << pueblo.getMejorError() << endl;
 pueblo.getMejorIndividuo()->ver();
 cout << pueblo.getErrorMedioPoblacion() << endl;
 cout << "Individuos creados: " << nTotalIndividuos << endl;
...
void ResuelveLirio()
{
 ...
 cout << "El error al usar el fichero de test es del " << errorTest << "%" << endl;
}
*/
...
void main10()
{
 ...
 tiempo.detente();
 cout << "Completadas " << pueblo.getNumGeneraciones() << " generaciones." << endl;
 cout << "Tiempo de evolucion: " << tiempo.detente() << " segundos." << endl;
 cout << "Error mejor/medio: " << pueblo.getMejorError() << "/" << pueblo.getErrorMedioPoblacion() << endl;
...
void ResuelveKozaRegresionSimbolica1()
{
 ...
 fstream fErrorMejor(nomFichErrorMejor, ios::out);		// Fichero de error del mejor individuo
 char *nomFichErrorMedio = generaNombre("eMedi000.dat");
 fstream fErrorMedio(nomFichErrorMedio, ios::out);		// Fichero de error medio de la poblacion
 ...
 while(nGener < 30 && !pueblo.errorAlcanzado())
 {
  ...
  fErrorMejor << errorMejor << endl;
  fErrorMedio << errorMedio << endl;
  /*
  // Cada 5 generaciones, imprimo el mejor y su simplificacion
  if(nGener % 5 == 0)
  {
   cout << "En la generacion numero " << nGener << ", el mejor es:" << endl;
   pueblo.getMejorIndividuo()->ver();
   cout << "Y su expresion simplificada es:" << endl;
 ...
 cout << "El error al usar el test ampliado es de " << errorTest << " (RMS)" << endl;
}
...
void ResuelveLentes()
{
 ...
 while(nGener < NUM_MAX_GENER && !pueblo.errorAlcanzado())
 {
  ...
  // Salva los errores de la generacion
  fError << errorMedio << " " << errorMejor << endl;
 }
 tiempo.detente();
 cout << "Tiempo de evolucion: " << tiempo.detente() << " segundos." << endl;
 cout << " Mejor: " << endl;
 // << *(pueblo.getMejorIndividuo()->getArbol()) << " : " << pueblo.getMejorError() << endl;
 pueblo.getMejorIndividuo()->ver();
 DEBUG("Pulsa!");
 pueblo.getMejorIndividuo()->verSimplificado();
 cout << pueblo.getErrorMedioPoblacion() << endl;
...
void funcionEscrituraExterna(ptrIndividuo pInd, ptrConjNodos pConjVariables, fstream &fichero)
{
 return;
 // Necesito conectar las variables del conjunto con una variable que reciba los datos del rango
 Variable *X = (Variable *) (*pConjVariables)[0];
 ...
 for(valX = -360.; valX < 360; valX++)
 {
  // Cargamos las variables del arbol con los valores correspondientes y lo evaluamos
  (*X) = valX;
  fichero << valX << "\t" << f(valX) << "\t" << pInd->evalua() << endl;
 }
