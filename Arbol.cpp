	...
	if(n)
	{
		...
		if(n->esOperacion())
		{
			...
			{                            // justo a continuacion en la pila, sin mas problemas
				...
				if(n->esBinario())		// Las binarias tienen el 1ñ parametro a continuacion
				{				// y el 2ñ como raiz de su subarbol derecho
					i->apilarPreOrden(vPila, numeroEntradas);
					vPila[indiceTemp].param2 = d->apilarPreOrden(vPila, numeroEntradas);
					vPila[indiceTemp].param3 = 0;
					vPila[indiceTemp].aridad = 2;	// Aridad 2
				}
				else			// Las ternarias tienen el 1ñ a continuacion, el 2ñ como
				{				// raiz de su subarbol central y el 3ñ en el derecho
					i->apilarPreOrden(vPila, numeroEntradas);
