/*
  ...
  \ToDo: Pasarlo a Tools.h en ingles...
*/
...
// El do-while es para poder tener un bloque totalmente independiente y poderlo usar en ifs...
#define DEBUG(mensaje)                          											\
    do {                                       												\
        std::cout << "DEBUG(" << "\033[31m" << mensaje << "\033[0m" << ")" << std::endl; 	\
        std::cout << "\tPress INTRO to continue..."; 										\
        std::cin.get(); 																	\
    } while (0)
...
// Intercambia entre si lo que sea de Tipo Generico TG.
// Si las comodas templates son lentas, hace una funcion para cada caso concreto de los que quiera intercambiar.
template <class TG>
void intercambia(TG &a, TG &b)      
{
 	TG temporal = a;
 	a = b;
 	b = temporal;
...
extern Flt f(const Flt &x); 						// Funcion de fitness?.
int fExtOrdenInd(const void *a, const void *b); 	// Para ordenar los individuos.
char *generaNombre(const char *ptrCad);				// Generador de nombre correlativos de ficheros.
